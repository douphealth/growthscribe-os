...
function article(extra = "") {
  const body = Array.from({ length: 180 }, (_, i) => `Useful evidence sentence ${i} explains the topic clearly.`).join(" ");
...
describe("Hermes autonomous growth", () => {
  ...
  it("rejects large content loss and fabricated first-person testing claims", () => {
    ...
    const result = validateContentRefresh({
      ...
      beforeExcerpt: "A practical explanation that helps readers understand the topic and make a better decision using the evidence already on the page.",
      beforeHtml: before,
      nextTitle: "A Complete Practical Guide for Readers",
      nextExcerpt: "A practical explanation that helps readers understand the topic and make a better decision using the evidence already on the page.",
